...
def show_recommendations():
    ...
    for i in range(len(tickers)):
        ...
        rounded_fuzzy_normalized_value = round(fuzzy_normalized_value, 2)

        str1 += "\t" + str(rounded_fuzzy_normalized_value)
        fuzzy = ""
        if 0<=rounded_fuzzy_normalized_value<=0.2:
            fuzzy = "Sell"
        elif 0.2<rounded_fuzzy_normalized_value<=0.4:
            fuzzy = "Underperforming"
        elif 0.4<rounded_fuzzy_normalized_value<=0.6:
            fuzzy = "Hold"
        elif 0.6<rounded_fuzzy_normalized_value<=0.8:
            fuzzy = "Buy"
        elif rounded_fuzzy_normalized_value>0.8:
            fuzzy = "Strong Buy"
        str1 += "\t" + fuzzy

    text1=scrolledtext.ScrolledText(frame3, height=13, width=45, bg='white',fg='green', font=('',16), bd=-2,wrap = WORD)
    text1.insert(END, str1)
